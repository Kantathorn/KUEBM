            ...
            <table className="table table-striped">
              <thead>
                <tr>
                  ...
                  <th className='fs-6' onClick={() => handleSort('collected_date')}>วันที่ต้องการยืม {getSortIcon('collected_date')}</th>
                    ...
                    <tr key={item._id}>
                      ...
                      <td>
                          <button className='btn btn-warning btn-sm' style={{width: "9rem"}} onClick={() => handleDetailClick(item)}>
                            {item.status === 'New' ? "จัดการคำร้อง" : item.status === 'Approve' ? "บันทึกการจัดส่งพัสดุ" : item.status === 'In-Use' ? "บันทึกการคืนพัสดุ" : "รายละเอียด"}
                          </button>
                      </td>
                    </tr>
                  )
